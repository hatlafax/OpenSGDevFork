...
OSG::NodeTransitPtr createScene()
{
    ...
    compShaderChunk->addUniformVariable("destTex", 0);
    compShaderChunk->addUniformVariable("roll",    0.f);
    compShaderChunk->setComputeProgram(get_cp_program());
    ...
    const OSG::ShaderVariableReal* varRealRoll  = dynamic_cast<const OSG::ShaderVariableReal*>(compShaderChunk->getVariables()->getVariable("roll"));
