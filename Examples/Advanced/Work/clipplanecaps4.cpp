...
OSG::ChunkMaterialTransitPtr createChunkMaterial(const OSG::Color3f& col)
{
    ...
    MaterialDescRefPtr matDesc = OSG::MaterialDesc::create();

    matDesc->setShadingModel    (MaterialDesc::BLINN_PHONG_SHADING_MODEL);
    matDesc->setFlatShadingMode (false);
    matDesc->setEmissive        (OSG::Color3f(0,0,0));
    matDesc->setAlbedo          (OSG::Color3f(    col[0],      col[1],      col[2]));
    matDesc->setSpecular        (OSG::Color3f(      0.8f,        0.8f,        0.8f));
    matDesc->setShininess       (20.f);
    matDesc->setOpacity         (1.0f);
    matDesc->setSRGBColorMode   (true);
    matDesc->setRoughness       (0.5f);
    matDesc->setMetalness       (0.8f);

    MaterialManager::ManagedMaterialT result = materialManager->addMaterial(matDesc);

    return ChunkMaterialTransitPtr(result.second);
}
...
void ClippingSceneManager::initShader(OSG::ShaderProgram* shader)
{
    if (shader && shader->getShaderType() == GL_VERTEX_SHADER)
    {
        ...
        for (int i = 0; i < 8; ++i)
        {
            clipplanes.push_back(OSG::Vec4f(1.f, 0.f, 0.f, 0.f));
            clipstate.push_back(false);
        }

        shader->addUniformVariable<OSG::MFVec4f> ("uClipPlanes", clipplanes);
        shader->addUniformVariable<OSG::MFInt32> ("uClipStates", clipstate);
    }
}

//----- updateShader ----------------------------------------------------------

void ClippingSceneManager::updateShader(OSG::ShaderProgram* shader)
{
    if (shader && shader->getShaderType() == GL_VERTEX_SHADER)
    {
        ...
        for (int i = 0;  i < 8; ++i)
        {
            if (i < sz)
            {
                const OSG::ClipPlaneChunkRecPtr& clipPlaneChunk = _vecClipPlaneDetails[i]._clipPlaneChunk;

                const OSG::Vec4f& plane_eqn = clipPlaneChunk->getEquation();
                bool               enabled   = clipPlaneChunk->getEnable();

                clipplanes.push_back(plane_eqn);
                clipstate .push_back(enabled);
            }
            else
            {
                clipplanes.push_back(OSG::Vec4f(1.f, 0.f, 0.f, 0.f));
        ...
        shader->updateUniformVariable<OSG::MFVec4f> ("uClipPlanes", clipplanes);
        shader->updateUniformVariable<OSG::MFInt32> ("uClipStates", clipstate);
    }
}
...
void ClippingSceneManager::createClipPlaneDetails()
{
    ...
    for(int i = 0; i < iNumClipPlanes; ++i)
    {
        ...
        // Define plane color
        //
        details._planeColor = Color3f(planeCol[i][0], planeCol[i][1], planeCol[i][2]);
...
*/
    ChunkMaterialUnrecPtr mat = createChunkMaterial(_vecClipPlaneDetails[i]._planeColor);
...
int doMain(int argc, char **argv)
{
    ...
    materialManager->setHasClipPlanes(true);
    materialManager->setWindow(pwin);
