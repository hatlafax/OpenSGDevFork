...
const OSG::MultiLightShadowStage::Mode ceShadowMode = OSG::MultiLightShadowStage::SIMPLE_TEST_SHADOW_MAP;
...
const OSG::UInt32 window_width     = 1600;//1017;  //1000;
const OSG::UInt32 window_height    = 1200;// 817;  //800;

const OSG::UInt32 num_materials    = 100;
const OSG::UInt32 max_num_lights   =   4;

const OSG::UInt32 num_geometries   = 12;        // 16;  // We show that number of objects
const OSG::UInt32 geom_start_index = 4;         // A number of objects are treated special

const OSG::Real32 circle_radius    = 4.f;       // 5.f;
const OSG::Real32 floor_size       = 100.f;
const OSG::Real32 center_box_size  = 1.f;       // if used: see
...
void Example::setupScene()
{
    ...
    centerTransNode->setCore(_geom_trans[1]);
    ...
    m.setIdentity();
    m.setTranslate(0.f, center_box_size, 0.f);
    ...
    centerGeo->setMaterial(geom_state[1]);
...
void Example::create_lights(OSG::UInt32 requiredShadowFeatures)
{
    ...
    _multiLightGroup->setUsage(GL_DYNAMIC_COPY);
    ...
    bool shadow_light2 = false;
    bool shadow_light3 = true;
    bool shadow_light4 = false;
...
void Example::setupTweakBar()
{
    ...
    TwEnumVal shadowModeEnums[] = {
        ...
        { OSG::MultiLightShadowStage::SIMPLE_TEST_SHADOW_MAP,   "Simple Test Shadow"},
    ...
    TwAddVarCB(_tweakbar, "Filter Radius",    TW_TYPE_FLOAT,   SetShadowFilterRadiusCB,     GetShadowFilterRadiusCB,    this, "min=0.0 max=100.0 step=0.01 precision=3 group='Shadow Parameter'");
...
std::string Example::get_fp_program()
{
    ...
    << endl << ""
    << endl << OSG::Shader::getEqualEpsSnippet()
    << endl << ""
    << _multiLightGroup->getFragmentProgramSnippet(true, true)
//######################################################################
    << endl << "bool testLightBuffer()"
    << endl << "{"
    << endl << "    bool flag = true;"
    << endl << "    mat4  mVal;"
    << endl << "    vec3  vVal;"
    ...
    << endl << "    bool  bVal;"
    << endl << ""
    << endl << "    vVal = lights.light[0].position;"
    << endl << "    if (OSGEqualEps(vVal,  vec3(0 ,4 ,3), 1.E-5) == false) flag = false;"
    << endl << "    vVal = lights.light[0].direction;"
    << endl << "    if (OSGEqualEps(vVal, vec3(0 ,0 ,1), 1.E-5) == false) flag = false;"
    << endl << "    vVal = lights.light[0].ambientIntensity;"
    << endl << "    if (OSGEqualEps(vVal, vec3(1 ,1 ,1), 1.E-5) == false) flag = false;"
    << endl << "    vVal = lights.light[0].diffuseIntensity;"
    << endl << "    if (OSGEqualEps(vVal, vec3(1 ,1 ,1), 1.E-5) == false) flag = false;"
    << endl << "    vVal = lights.light[0].specularIntensity;"
    << endl << "    if (OSGEqualEps(vVal, vec3(1 ,1 ,1), 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[0].constantAttenuation;"
    << endl << "    if (OSGEqualEps(fVal, 1, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[0].linearAttenuation;"
    << endl << "    if (OSGEqualEps(fVal, 0.0001, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[0].quadraticAttenuation;"
    << endl << "    if (OSGEqualEps(fVal, 1e-006, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[0].rangeCutOn;"
    << endl << "    if (OSGEqualEps(fVal, 0.1, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[0].rangeCutOff;"
    << endl << "    if (OSGEqualEps(fVal, 9.99e+006, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[0].cosSpotlightAngle;"
    << endl << "    if (OSGEqualEps(fVal, 1, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[0].spotlightExponent;"
    << endl << "    if (OSGEqualEps(fVal, 1, 1.E-5) == false) flag = false;"
    << endl << "    iVal = lights.light[0].type;"
    << endl << "    if (iVal != 1) flag = false;"
    << endl << "    bVal = lights.light[0].enabled;"
    << endl << "    if (bVal != true) flag = false;"
    << endl << "    bVal = lights.light[0].shadow;"
    << endl << "    if (bVal != true) flag = false;"
    << endl << "    iVal = lights.light[0].shadowDataIndex;"
    << endl << "    if (iVal != 0) flag = false;"
    << endl << "    iVal = lights.light[0].shadowParameterIndex;"
    << endl << "    if (iVal != 0) flag = false;"
    << endl << ""
    << endl << "    vVal = lights.light[1].position;"
    << endl << "    if (OSGEqualEps(vVal,  vec3(0 ,7 ,3), 1.E-5) == false) flag = false;"
    << endl << "    vVal = lights.light[1].direction;"
    << endl << "    if (OSGEqualEps(vVal, vec3(0 ,-0.919145 ,-0.393919), 1.E-5) == false) flag = false;"
    << endl << "    vVal = lights.light[1].ambientIntensity;"
    << endl << "    if (OSGEqualEps(vVal, vec3(1 ,1 ,1), 1.E-5) == false) flag = false;"
    << endl << "    vVal = lights.light[1].diffuseIntensity;"
    << endl << "    if (OSGEqualEps(vVal, vec3(1 ,1 ,1), 1.E-5) == false) flag = false;"
    << endl << "    vVal = lights.light[1].specularIntensity;"
    << endl << "    if (OSGEqualEps(vVal, vec3(1 ,1 ,1), 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[1].constantAttenuation;"
    << endl << "    if (OSGEqualEps(fVal, 1, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[1].linearAttenuation;"
    << endl << "    if (OSGEqualEps(fVal, 0.0001, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[1].quadraticAttenuation;"
    << endl << "    if (OSGEqualEps(fVal, 1e-006, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[1].rangeCutOn;"
    << endl << "    if (OSGEqualEps(fVal, 0.1, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[1].rangeCutOff;"
    << endl << "    if (OSGEqualEps(fVal, 9.99e+006, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[1].cosSpotlightAngle;"
    << endl << "    if (OSGEqualEps(fVal, 0.819152, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[1].spotlightExponent;"
    << endl << "    if (OSGEqualEps(fVal, 1, 1.E-5) == false) flag = false;"
    << endl << "    iVal = lights.light[1].type;"
    << endl << "    if (iVal != 3) flag = false;"
    << endl << "    bVal = lights.light[1].enabled;"
    << endl << "    if (bVal != false) flag = false;"
    << endl << "    bVal = lights.light[1].shadow;"
    << endl << "    if (bVal != false) flag = false;"
    << endl << "    iVal = lights.light[1].shadowDataIndex;"
    << endl << "    if (iVal != -1) flag = false;"
    << endl << "    iVal = lights.light[1].shadowParameterIndex;"
    << endl << "    if (iVal != 1) flag = false;"
    << endl << ""
    << endl << "    vVal = lights.light[2].position;"
    << endl << "    if (OSGEqualEps(vVal,  vec3(-0.654 ,0.589 ,0.476), 1.E-5) == false) flag = false;"
    << endl << "    vVal = lights.light[2].direction;"
    << endl << "    if (OSGEqualEps(vVal, vec3(0.653604 ,-0.588643 ,-0.475712), 1.E-5) == false) flag = false;"
    << endl << "    vVal = lights.light[2].ambientIntensity;"
    << endl << "    if (OSGEqualEps(vVal, vec3(1 ,1 ,1), 1.E-5) == false) flag = false;"
    << endl << "    vVal = lights.light[2].diffuseIntensity;"
    << endl << "    if (OSGEqualEps(vVal, vec3(1 ,1 ,1), 1.E-5) == false) flag = false;"
    << endl << "    vVal = lights.light[2].specularIntensity;"
    << endl << "    if (OSGEqualEps(vVal, vec3(1 ,1 ,1), 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[2].constantAttenuation;"
    << endl << "    if (OSGEqualEps(fVal, 1, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[2].linearAttenuation;"
    << endl << "    if (OSGEqualEps(fVal, 0.0001, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[2].quadraticAttenuation;"
    << endl << "    if (OSGEqualEps(fVal, 1e-006, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[2].rangeCutOn;"
    << endl << "    if (OSGEqualEps(fVal, 0.1, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[2].rangeCutOff;"
    << endl << "    if (OSGEqualEps(fVal, 1000, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[2].cosSpotlightAngle;"
    << endl << "    if (OSGEqualEps(fVal, 1, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[2].spotlightExponent;"
    << endl << "    if (OSGEqualEps(fVal, 1, 1.E-5) == false) flag = false;"
    << endl << "    iVal = lights.light[2].type;"
    << endl << "    if (iVal != 2) flag = false;"
    << endl << "    bVal = lights.light[2].enabled;"
    << endl << "    if (bVal != true) flag = false;"
    << endl << "    bVal = lights.light[2].shadow;"
    << endl << "    if (bVal != true) flag = false;"
    << endl << "    iVal = lights.light[2].shadowDataIndex;"
    << endl << "    if (iVal != 2) flag = false;"
    << endl << "    iVal = lights.light[2].shadowParameterIndex;"
    << endl << "    if (iVal != 2) flag = false;"
    << endl << ""
    << endl << "    vVal = lights.light[3].position;"
    << endl << "    if (OSGEqualEps(vVal,  vec3(-0.654 ,0.589 ,-0.476), 1.E-5) == false) flag = false;"
    << endl << "    vVal = lights.light[3].direction;"
    << endl << "    if (OSGEqualEps(vVal, vec3(0.653604 ,-0.588643 ,0.475712), 1.E-5) == false) flag = false;"
    << endl << "    vVal = lights.light[3].ambientIntensity;"
    << endl << "    if (OSGEqualEps(vVal, vec3(1 ,1 ,1), 1.E-5) == false) flag = false;"
    << endl << "    vVal = lights.light[3].diffuseIntensity;"
    << endl << "    if (OSGEqualEps(vVal, vec3(1 ,1 ,1), 1.E-5) == false) flag = false;"
    << endl << "    vVal = lights.light[3].specularIntensity;"
    << endl << "    if (OSGEqualEps(vVal, vec3(1 ,1 ,1), 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[3].constantAttenuation;"
    << endl << "    if (OSGEqualEps(fVal, 1, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[3].linearAttenuation;"
    << endl << "    if (OSGEqualEps(fVal, 0.0001, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[3].quadraticAttenuation;"
    << endl << "    if (OSGEqualEps(fVal, 1e-006, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[3].rangeCutOn;"
    << endl << "    if (OSGEqualEps(fVal, 0.1, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[3].rangeCutOff;"
    << endl << "    if (OSGEqualEps(fVal, 1000, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[3].cosSpotlightAngle;"
    << endl << "    if (OSGEqualEps(fVal, 1, 1.E-5) == false) flag = false;"
    << endl << "    fVal = lights.light[3].spotlightExponent;"
    << endl << "    if (OSGEqualEps(fVal, 1, 1.E-5) == false) flag = false;"
    << endl << "    iVal = lights.light[3].type;"
    << endl << "    if (iVal != 2) flag = false;"
    << endl << "    bVal = lights.light[3].enabled;"
    << endl << "    if (bVal != false) flag = false;"
    << endl << "    bVal = lights.light[3].shadow;"
    << endl << "    if (bVal != false) flag = false;"
    << endl << "    iVal = lights.light[3].shadowDataIndex;"
    << endl << "    if (iVal != -1) flag = false;"
    << endl << "    iVal = lights.light[3].shadowParameterIndex;"
    << endl << "    if (iVal != 3) flag = false;"
    << endl << ""
    << endl << "    return flag;"
    << endl << "}"
    ...
    << endl << ""
    << endl << "const vec3 test_color = vec3(1,0,0);"
    << endl << ""
    << endl << "Light current_light;"
    << endl << ""
    << endl << "//"
    ...
    << endl << "    //"
    << endl << "    vec4 direction = OSGViewMatrix * vec4(current_light.direction, 0.0);"
    ...
    << endl << "    vec3 Ia = current_light. ambientIntensity;"
    << endl << "    vec3 Id = current_light. diffuseIntensity;"
    << endl << "    vec3 Is = current_light.specularIntensity;"
    << endl << ""
    ...
    << endl << ""
    << endl << "    float shadow = shadowAttenuationDirectionalLight(p.z, p_ws, n_ws, current_light);"
    << endl << "    float attenuation = shadow;"
    << endl << ""
    ...
    << endl << "                            current_light. constantAttenuation,"
    << endl << "                            current_light.   linearAttenuation,"
    << endl << "                            current_light.quadraticAttenuation,"
    << endl << "                            d);"
    << endl << ""
    << endl << "    vec3 Ke = materials.material[j].emissive;"
    << endl << "    vec3 Ka = materials.material[j].ambient;"
    ...
    << endl << "    vec3 Cs = Is * Ks * (m+8) * 0.0125 * pf;"
    << endl << ""
    << endl << "    float shadow = shadowAttenuationPointLight(p.z, p_ws, n_ws, current_light);"
    << endl << "    attenuation *= shadow;"
    << endl << ""
    << endl << "    if (uRenderEmissive) result += Ce;"
    << endl << "    if (uRenderAmbient)  result += Ca;"
    << endl << "    if (uRenderDiffuse)  result += attenuation * Cd;"
    ...
    << endl << "                            d);"
    << endl << ""
    << endl << "    attenuation *= spotAttenuation(current_light.cosSpotlightAngle, current_light.spotlightExponent, l, s);"
    << endl << ""
    << endl << "    vec3 Ke = materials.material[j].emissive;"
    << endl << "    vec3 Ka = materials.material[j].ambient;"
    << endl << "    vec3 Kd = materials.material[j].diffuse;"
    ...
    << endl << "    vec3 Cs = Is * Ks * (m+8) * 0.0125 * pf;"
    << endl << ""
    << endl << "    float shadow = shadowAttenuationSpotLight(p.z, p_ws, n_ws, current_light);"
    << endl << "    attenuation *= shadow;"
    << endl << ""
    << endl << "    if (uRenderEmissive) result += Ce;"
    << endl << "    if (uRenderAmbient)  result += Ca;"
    ...
    << endl << ""
    << endl << "    bool test_flag = testLightBuffer();"
    << endl << ""
    << endl << "    for (int i = 0; i < num_lights; ++i)"
    << endl << "    {"
    << endl << "        current_light = lights.light[i];"
    << endl << ""
    << endl << "        bool enabled = current_light.enabled;"
    << endl << "        int  type    = current_light.type;"
    << endl << "        uint value   = k & (1 << i);"
    << endl << ""
    << endl << "        if ( !enabled || value != 0 ) continue;"
    << endl << ""
    << endl << "        if (uRenderSplitting) color += shadowSplitIndexTest(vPositionES.z, current_light).xyz;"
    << endl << ""
    << endl << "        switch (type)"
    << endl << "        {"
    << endl << "            case       POINT_LIGHT: color +=       pointLight(j, N_ES, N_WS, V, vPositionES, vPositionWS);  break;"
    << endl << "            case DIRECTIONAL_LIGHT: color += directionalLight(j, N_ES, N_WS, V, vPositionES, vPositionWS);  break;"
    << endl << "            case        SPOT_LIGHT: color +=        spotLight(j, N_ES, N_WS, V, vPositionES, vPositionWS);  break;"
    << endl << "        }"
    //<< endl << "        test_flag = test_flag && shadowTestShader(i, current_light);"
    << endl << "    }"
    << endl << ""
    //<< endl << "    if (test_flag)"
    //<< endl << "        color = vec3(0,1,0);"
    //<< endl << "    else"
    //<< endl << "        color = vec3(1,0,0);"
    << endl << ""
    << endl << "    float opacity = materials.material[j].opacity;"
    << endl << ""
    << endl << "    vFragColor = vec4(color, opacity);"
    << endl << "}"
    << endl << ""
    << endl;
