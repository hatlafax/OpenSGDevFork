...
OSG::ShaderProgramTransitPtr Example::createFragShader()
{
    ...
    // Test only
    if (_shadowStage->getDisableOverride() == true)
    {
        OSG::Int32 iPLTextureUnit       = 31;//_shadowStage->getSamplerCubeArrayTextureUnit();
        OSG::Int32 iDLTextureUnit       = 30;//_shadowStage->getSampler2DArrayTextureUnit();
        OSG::Int32 iPLShadowTextureUnit = 29;//_shadowStage->getSamplerCubeArrayShadowTextureUnit();
        OSG::Int32 iDLShadowTextureUnit = 28;//_shadowStage->getSampler2DArrayShadowTextureUnit();
        OSG::Int32 iShadowDataDBndPnt   =  7;//_shadowStage->getShadowDataBindingPnt();
        OSG::Int32 iShadowParamBndPnt   = 13;//_shadowStage->getShadowParameterBindingPnt();
        OSG::Int32 iDistributionBndPnt  = 12;//_shadowStage->getDistributionBindingPnt();

        fragShader->addUniformVariable   ("uShadowEnabled",       _shadowStage->getShadowOn());
        fragShader->addUniformVariable   ("uShadowMapPnt",        iPLTextureUnit);
        fragShader->addUniformVariable   ("uShadowMapDir",        iDLTextureUnit);
        fragShader->addUniformVariable   ("uShadowMapPntComp",    iPLShadowTextureUnit);
        fragShader->addUniformVariable   ("uShadowMapDirComp",    iDLShadowTextureUnit);
        fragShader->addShaderStorageBlock("ShadowDataBlock",      OSG::UInt32(iShadowDataDBndPnt));
        fragShader->addUniformBlock      ("ShadowParameterBlock", OSG::UInt32(iShadowParamBndPnt));
        fragShader->addUniformBlock      ("DistributionBlock",    OSG::UInt32(iDistributionBndPnt));

        fragShader->addUniformVariable   ("uUseShadowSampler",    _shadowStage->getUseHWPCF());
    }
...
//#define NO_HDR_STAGE
    ...
    // Test only
    _shadowStage->setDisableOverride(true);
...
void Example::SetShadowOn(bool value)
{
    ...
    if (_shadowStage->getDisableOverride() == true)
    {
        _fragShader->updateUniformVariable   ("uShadowEnabled",       _shadowStage->getShadowOn());
    }
...
void Example::SetShadowUseHwPCF(bool value)
{
    ...
    if (_shadowStage->getDisableOverride() == true)
    {
        _fragShader->updateUniformVariable   ("uUseShadowSampler",    _shadowStage->getUseHWPCF());
    }
...
std::string Example::get_fp_program()
{
    ...
    if (_shadowStage->getDisableOverride() == false)
    {
    ...
    else
    {
        ...
        << endl << "    vec2 filterDisk[32];"
        << endl << "    vec2 searchDisk[25];"
        << endl << "} distribution;"
        << endl << ""
        << endl << "vec3 getShadowCoords(in mat4 matTSFromWS, in vec3 fragPosWS)"
        << endl << "{"
        ...
        << endl << "}"
        << endl << ""
        << endl << "const int OSG_POSITIVE_X = 0;"
        ...
        << endl << "}"
        << endl << ""
        << endl << "vec3 OSGConvertCubeUV2XYZ(in int index, in vec2 uv)"
        << endl << "{"
        << endl << "    float uc = 2.0 * uv.x - 1.0;"
        << endl << "    float vc = 2.0 * uv.y - 1.0;"
        << endl << ""
        << endl << "    vec3 xyz;"
        << endl << "    if (index == OSG_POSITIVE_X) return vec3( 1.0,  vc, -uc);"
        << endl << "    if (index == OSG_NEGATIVE_X) return vec3(-1.0,  vc,  uc);"
        << endl << "    if (index == OSG_POSITIVE_Y) return vec3(  uc, 1.0, -vc);"
        << endl << "    if (index == OSG_NEGATIVE_Y) return vec3(  uc,-1.0,  vc);"
        << endl << "    if (index == OSG_POSITIVE_Z) return vec3(  uc,  vc, 1.0);"
        << endl << "    if (index == OSG_NEGATIVE_Z) return vec3( -uc,  vc,-1.0);"
        << endl << ""
        << endl << "    return vec3(0);"
        << endl << "}"
        << endl << ""
        << endl << "vec2 OSGConvertXYZ2CubeUV(in vec3 s, out int index)"
        << endl << "{"
        << endl << "    float absX = abs(s.x);"
        << endl << "    float absY = abs(s.y);"
        << endl << "    float absZ = abs(s.z);"
        ...
        << endl << "        return texture(uShadowMapDirComp, vec4(samplePos, ref)).x;"
        << endl << "    else"
        ...
        << endl << "    vec4 samplePos = vec4(dir, layer);"
        << endl << "    if (uUseShadowSampler)"
        << endl << "        return texture(uShadowMapPntComp, samplePos, ref).x;"
        << endl << "    else"
        ...
        << endl << ""
        << endl << "    const int N  = distribution.boxSearchSize / 2;"
        << endl << "    const int N2 = 2 * N;"
        ...
        << endl << "            blockerDepth += test * depth;"
        << endl << "            numBlockers  += test;"
        << endl << "        }"
        << endl << "    }"
        << endl << ""
        ...
        << endl << "    float shadow = 0.0;"
        << endl << ""
        << endl << "    const int N  = distribution.boxFilterSize / 2;"
        << endl << "    const int N2 = 2 * N;"
        << endl << "    float anchor = float(N) - 0.5;"
        ...
        << endl << "                        : fp.filterRadius;"
        << endl << ""
        << endl << "    const int kernelSize   = distribution.searchDisk.length();"
        << endl << "    const int kernelOffset = 0;"
        << endl << ""
        ...
        << endl << "    float shadow = 0.0;"
        << endl << ""
        << endl << "    const int kernelSize   = distribution.filterDisk.length();"
        << endl << "    const int kernelOffset = 0;"
        << endl << ""
        ...
        << endl << "}"
        << endl << "const int PCF_MODE_DISABLED = 0;"
        ...
        << endl << "    float bias     = param.bias * clamp(factor, 0.01, 100.0);"
        << endl << ""
        ...
        << endl << "                                        sin_a, cos_a);"
        << endl << ""
        << endl << "    return calcPCFShadowPnt("
        << endl << "                    param.filterMode,"
        << endl << "                    layer,"
        << endl << "                    projDir,"
        << endl << "                    filterParam) * param.intensity;"
        << endl << "}"
        << endl << ""
        ...
        << endl << "                                        sin_a, cos_a);"
        << endl << ""
        << endl << "    return calcPCFShadowDir("
        << endl << "                    param.filterMode,"
        << endl << "                    layer,"
        << endl << "                    uv,"
        << endl << "                    filterParam) * param.intensity;"
        << endl << "}"
        << endl << "float shadowAttenuationDirectionalLight("
        << endl << "    in float fragDepthES,"
        << endl << "    in vec3  fragPosWS,"
        << endl << "    in vec3  normalWS,"
        << endl << "    in Light light)"
