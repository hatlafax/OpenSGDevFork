// OpenSG Tutorial Example: MultiLightGroup
//
// This example introduces the MultiLightGroup that wraps a MultiLightChunk.
// It possess the same interface as the MultiLightChunk and is just a 
// convenience wrapper class.
// The example follows the MultiRangeLight example. Further information can
...
//
// The MultiLightGroup is able to provide fragment shader code snippets
...
// get_fp_program.
//
#define USE_CODE_FROM_MULTI_LIGHT_GROUP
...
const OSG::UInt32 materialBindingPnt = 1;
const OSG::UInt32 lightBindingPnt    = 2;
const OSG::UInt32 geomBindingPnt     = 3;
...
struct Light
{
    ...
    OSG::Vec3f   intensity;         // the color intensity of the light
...
OSG::MultiLightGroupTransitPtr create_light_state(const VecLightsT& vLights)
{
    OSG::MultiLightGroupRefPtr lightGroup = OSG::MultiLightGroup::create();

    lightGroup->setUsage(GL_DYNAMIC_DRAW);
    lightGroup->setLayoutType(OSG::MultiLight::SIMPLE_LAYOUT);
    lightGroup->setLightBindingPnt(lightBindingPnt);

    BOOST_FOREACH(const Light& light, vLights)
    {
        OSG::UInt32 idx = lightGroup->addLight(light.getTypeOfLight());

        lightGroup->setPosition        (idx, light.position);
        lightGroup->setDirection       (idx, light.direction);
        lightGroup->setIntensity       (idx, light.intensity);
        lightGroup->setRange           (idx, light.range);
        lightGroup->setSpotlightAngle  (idx, light.spotlightAngle);
        lightGroup->setEnabled         (idx, light.enabled);
        lightGroup->setTypeOfLight     (idx, light.getTypeOfLight());
        lightGroup->setBeacon          (idx, light.beacon);
    }

    return OSG::MultiLightGroupTransitPtr(lightGroup);
}

void update_light_state(OSG::MultiLightGroup* lightGroup, const VecLightsT& vLights)
{
    if (lightGroup)
    {
        if (lightGroup->getNumLights() != vLights.size())
        {
            lightGroup->clearLights();
...
//
// The mulit light group
//
OSG::NodeRefPtr multi_light_node = NULL;
OSG::MultiLightGroupRefPtr multi_light_group = NULL;
    ...
    OSG::UInt32 material_idx)   // index in to the material storage for light animation
{
    ...
    switch (e)
    {
        case point_light:
        {
            ...
            geom_state->addChunk(ubo_geom_state, geomBindingPnt);   // buffer binding point 3
...
int main(int argc, char **argv)
{
    ...
    // Otherwise OpenSG will complain about objects being alive after shutdown.
    {
        ...
        multi_light_node   = OSG::makeCoredNode<OSG::Group>();
        scene_node         = OSG::makeCoredNode<OSG::Group>();

        mgr->setRoot(hdr_node);
        hdr_node->addChild(multi_light_node);
        multi_light_node->addChild(scene_node);
        ...
        //
        fragShader->addShaderStorageBlock("Materials", materialBindingPnt);       // block binding point
        fragShader->addShaderStorageBlock("Lights",    lightBindingPnt);          // block binding point

        //
        // The following is replaced by adding ShaderProgramVariableChunk objects
        // to the chunk material. See below...
        //
        // fragShader->addShaderStorageBlock("GeomState", geomBindingPnt);        // block binding point
        ...
        shader_var_chunk->addUniformBlock("GeomState", geomBindingPnt);    // block binding point
        ...
        //
        // Create the multi light group from the initialized light list
        //
        multi_light_group = create_light_state(lights);
        multi_light_node->setCore(multi_light_group);
        ...
        //
        // the following statements must be done after the creation of the multi_light_group
        ...
        prog_state->addChunk(ssbo_material_database, materialBindingPnt);
...
void keyboard(unsigned char k, int x, int y)
{
    switch(k)
    {
        case 27:        // ESC
        {
            ...
            multi_light_node = NULL;
            multi_light_group = NULL;
        ...
        {
            initialize_lights(1);
            update_light_state(multi_light_group, lights);
...
std::string get_fp_program()
{
    ...
    << endl << ""
#ifdef USE_CODE_FROM_MULTI_LIGHT_GROUP
    << multi_light_group->getFragmentProgramSnippet(true, true)
#else
